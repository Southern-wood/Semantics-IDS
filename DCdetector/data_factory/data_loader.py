...
class SWATSegLoader(Dataset):
    def __init__(self, normal_path, attack_path, labels_path, win_size=105, step=1, mode="train"):
        self.mode = mode
        self.step = step
        self.win_size = win_size
        self.scaler = StandardScaler()

        data = np.load(normal_path, allow_pickle=True)
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        self.train = data
        
        test_data = np.load(attack_path, allow_pickle=True)
        self.test = self.scaler.transform(test_data)
        # self.train = data[:(int)(len(data) * 0.8)]
        self.val = data[(int)(len(data) * 0.8):]
        self.test_labels = np.load(labels_path, allow_pickle=True)
        self.test_labels = np.any(self.test_labels == 1, axis=1).astype(np.float32)
        self.flag = mode
        print("test:", self.test.shape)
        print("train:", self.train.shape)
...
class WADISegLoader(Dataset):
    ...
    def __init__(self, root_path, win_size, step=1, flag="train", attack_path=None, labels_path=None):
        self.flag = flag
        self.step = step
        self.win_size = win_size
        self.scaler = StandardScaler()
        ...
        print('train path:',os.path.join(root_path, 'train','pure_train_0'))
        #强制训练数据的路径
        train_data = np.load(os.path.join('/data/processed/WADI/train/pure_train_0_num_0.npy')).astype(np.float32)
        # train_data = np.load(os.path.join(root_path, 'train','pure_train_0_num_0.npy')).astype(np.float32)

        print('test path:',os.path.join(root_path))
        test_data = np.load(os.path.join(attack_path)).astype(np.float32)
        # test_data = np.load(os.path.join(root_path)).astype(np.float32)#concealment_0/unconstrained_spoofing/pure_replay.npy
        labels = np.load(os.path.join(labels_path)).astype(np.float32)
        labels = np.any(labels == 1, axis=1).astype(np.float32)
    ...
    def __len__(self):
        ...
        """
        if self.flag == "train":
            return (self.train.shape[0] - self.win_size * self.step) // self.step + 1
        elif (self.flag == 'val'):
            return (self.val.shape[0] - self.win_size * self.step) // self.step + 1
        elif (self.flag == 'test'):
            return (self.test.shape[0] - self.win_size * self.step) // self.step + 1
        else:
            print('thre num',(self.test.shape[0] - self.win_size * self.step) // (self.win_size * self.step) + 1)
            return (self.test.shape[0] - self.win_size * self.step) // (self.win_size * self.step) + 1

    def __getitem__(self, index):
        index = index * self.step
        if self.flag == "train":
            return np.float32(self.train[index:index+self.win_size*self.step:self.step]), np.float32(self.test_labels[0:self.win_size])
        elif (self.flag == 'val'):
            return np.float32(self.val[index:index+self.win_size*self.step:self.step]), np.float32(self.test_labels[0:self.win_size])
        elif (self.flag == 'test'):
            return np.float32(self.test[index:index+self.win_size*self.step:self.step]), np.float32(
                self.test_labels[index:index+self.win_size*self.step:self.step])
        else:
...
def get_loader_segment(normal_path, attack_path, labels_path, batch_size, win_size=105, mode='train', dataset='KDD'):
    # if (dataset == 'SMD'):
    ...
    dataset = SWATSegLoader(normal_path, attack_path, labels_path, win_size, 1, mode)
    # elif (dataset == 'WADI'):
    #     dataset = WADISegLoader(normal_path, attack_path, labels_path, win_size, 1, mode)
    # elif (dataset == 'UCR'):
