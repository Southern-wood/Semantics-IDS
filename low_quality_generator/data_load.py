...
def load_SWaT(dataset_folder):
    ...
    column_names = ['Timestamp', 'FIT101', 'LIT101', 'MV101', 'P101', 'P102', 'AIT201',
    ...
    attack_pd['Timestamp'] = pd.to_datetime(attack_pd['Timestamp'],dayfirst=True,errors='coerce')
    normal_pd['Timestamp'] = pd.to_datetime(normal_pd['Timestamp'],dayfirst=True,errors='coerce')
    ...
    for i in attack_pd.columns.tolist()[1:-1]: labels[i] = 0
    ...
    return attack_pd, normal_pd, labels


def load_WADI(dataset_folder):
    dataset_folder = os.path.join(dataset_folder, 'WADI')
    train = pd.read_csv(os.path.join(dataset_folder, 'WADI_14days.csv'), skiprows=4) #4 head lines are uesless  nrows=2e5+996
    test = pd.read_csv(os.path.join(dataset_folder, 'WADI_attackdata.csv'))
    ls = pd.read_csv(os.path.join(dataset_folder, 'WADI_attackdata_labels.csv'))
    ...
    #Check the train and test original column names are same, remove the redunant columns (only one category value) and locate the category columns indexs
